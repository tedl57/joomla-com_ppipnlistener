...
while( !feof( $fp ) )
{
	...
	if (strcmp ($res, "VERIFIED"."\r\n" ) == 0) // todo: not sure why paypal is adding ^M (CR=\r) to all 
	{
		$verified = true;
...
// 2. check the transaction is not already marked paid (or completed or cancelled?)
// 3. check the transaction amount and the paid amount match
// 4. check the custom or "items" match?
...
function putLogEntry( $txt )
{
	...
	$result = JFactory::getDbo()->insertObject('#__ppipnlistener_log',
		$session, 'id');
}
